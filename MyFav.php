...
include('connect.php');
    ...
    $userId = $_SESSION['id'];
    $q="SELECT * FROM `produits` p INNER JOIN `favoris` f on f.produitId = p.PrimarykeyProduits INNER JOIN `nutriscore` n on n.PrimarykeyNutriScore = p.NuritionalScoreId WHERE userId = $userId";
