...
foreach (range('A', 'Z') as $alphabet) {?>
    <a class="col-12 AlphaBet "  href="NutritionalFacts.php?alphabet=<?php echo $alphabet ?>" style="text-decoration:none;color:black;background-color:#008c00;background-image: linear-gradient(to right,green,yellow);border-bottom:1px solid black;height: 46px;padding: 10px;">
   <?php echo $alphabet ?>
...
?>
  ...
  if(isset($_GET['alphabet'] ) && isset($_GET['alphabet'] ) != ""){
     $value=$_GET['alphabet'];
     $q="SELECT DISTINCT i.nameIngred  FROM nutritionfacts n INNER JOIN ingredient i on n.ingredientId=i.PrimarykeyIngredient WHERE i.nameIngred LIKE '$value%'";
