...
foreach (range('A', 'Z') as $alphabet) {?>
    <a href="Allergies.php?alphabet=<?php echo $alphabet ?>" class="col-12 AlphaBet" style="text-decoration:none;color:black;background-color:#008c00;background-image: linear-gradient(to right,green,yellow);border-bottom:1px solid black;height: 46px;padding: 10px;">
   <?php echo $alphabet ?>
...
?>
  ...
  if(isset($_GET['alphabet'] ) && isset($_GET['alphabet'] ) != ""){
     $value=$_GET['alphabet'];
     $q="SELECT  DISTINCT i.nameIngred  FROM allergie a INNER JOIN ingredient i on a.ingredientId=i.PrimarykeyIngredient WHERE i.nameIngred LIKE '$value%'";
